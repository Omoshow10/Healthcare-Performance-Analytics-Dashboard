...
ORDER BY avg_readmission_rate_pct DESC NULLS LAST;
...
SELECT
    ...
    ROUND(PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY fr.readmission_rate_pct), 2) AS p25,
    ROUND(PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY fr.readmission_rate_pct), 2) AS median,
    ROUND(PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY fr.readmission_rate_pct), 2) AS p75
FROM core.fact_readmissions fr
JOIN core.dim_hospital h ON fr.hospital_key = h.hospital_key
...
SELECT
    ...
    h.hospital_type,
    h.patient_exp_group,
    -- Overall rating (H-HSP-RATING-9-10)
    MAX(CASE WHEN m.measure_id = 'H-HSP-RATING-9-10'
             THEN fs.star_rating END)               AS overall_star_rating,
    -- Recommend hospital (H-RECMND-DY)
    MAX(CASE WHEN m.measure_id = 'H-RECMND-DY'
             THEN fs.answer_percent END)             AS pct_recommend,
    -- Composite domain scores
    ROUND(AVG(CASE WHEN m.measure_id IN ('H-COMP-1-A-P','H-COMP-2-A-P','H-COMP-3-A-P','H-COMP-5-A-P')
...
ORDER BY overall_star_rating DESC NULLS LAST, pct_recommend DESC NULLS LAST;

-- B2. HCAHPS Domain Scores — National & State Averages
-- ────────────────────────────────────────────────────
SELECT
    h.state,
    m.measure_id,
    ...
    ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP
          (ORDER BY fs.answer_percent), 1)      AS median_pct_always
FROM core.fact_patient_satisfaction fs
JOIN core.dim_hospital h ON fs.hospital_key = h.hospital_key
JOIN core.dim_measure  m ON fs.measure_key  = m.measure_key
JOIN core.dim_date     d ON fs.end_date_key = d.date_key
WHERE m.measure_category = 'Patient Experience'
  AND d.year = (SELECT MAX(year) FROM core.dim_date
                WHERE date_key IN (SELECT end_date_key FROM core.fact_patient_satisfaction))
GROUP BY h.state, m.measure_id, m.measure_name
...
-- ─────────────────────────────────────────────────────────────────────────────
-- SECTION C: CORRELATION ANALYSIS — Readmission Rate vs Patient Satisfaction
-- ─────────────────────────────────────────────────────────────────────────────

-- C1. Hospital-Level: Readmission Rate vs Overall Star Rating
-- ────────────────────────────────────────────────────────
WITH readmit AS (
    SELECT
        fr.hospital_key,
...
satisfaction AS (
    SELECT
        ...
        MAX(CASE WHEN m.measure_id = 'H-HSP-RATING-9-10'
                 THEN fs.star_rating END) AS overall_star_rating,
        MAX(CASE WHEN m.measure_id = 'H-RECMND-DY'
                 THEN fs.answer_percent END) AS pct_would_recommend
    FROM core.fact_patient_satisfaction fs
    JOIN core.dim_measure  m ON fs.measure_key  = m.measure_key
    JOIN core.dim_date     d ON fs.end_date_key = d.date_key
    WHERE d.year = (SELECT MAX(year) FROM core.dim_date
